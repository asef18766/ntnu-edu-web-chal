...
function submit() {
    fetch(`${SERVER_URL}/api/submit`, {
        ...
        body: JSON.stringify({ comment: review.value })
    })
    .then(() => {
        alert("您的回饋已提交給老師囉！")
        router.push('/view')
...
<template>
    <div class="h-full w-full">
        <div class="text-xl font-bold">填寫</div>
        <div class="form-control">
            <label class="label">
                <span class="label-text">課程回饋</span>
            </label>
            <div
        ...
        <div class=" mt-2" />
        <div
            :class="['inline-block animate-border rounded-xl bg-white from-teal-500 via-purple-500 to-pink-500 bg-[length:400%_400%] p-0.5 transition bg-gradient-to-r shadow-lg focus:outline-none focus:ring', review === '' && 'cursor-not-allowed']"
        >
            <button :class="['btn btn-wide btn-ghost text-white', review === '' && 'btn-disabled cursor-not-allowed']" @click="submit">Submit</button>
        </div>
        <p class="mt-4">
            親愛的同學：<br />
